...
public class IndexCountServiceImpl implements IndexCountService {
  ...
  public Map<String, Integer> getPanelGroupDataCount(String uid) {
    String json=redisTemplate.opsForValue().get("index_PanelGroupDataCount_"+uid);
    if(StringUtils.isBlank(json)) {
      Map<String, Integer> map = Maps.newHashMap();
      Integer processNum = indexCountDao.getProcessNumSum(uid);
      Integer taskNum = indexCountDao.getTaskNum(uid);
      Integer exceptionNum = indexCountDao.getExceptionNum(uid);
      ...
      redisTemplate.opsForValue().set("index_PanelGroupDataCount_"+uid, JSON.toJSONString(map));
      redisTemplate.expire("index_PanelGroupDataCount_"+uid, 1, TimeUnit.MINUTES);
      return map;
    }else{
